...
spec = do
  describe "eval'" $ do
    it "evals `[clone] clone`" $ do
      ...
      let (Right vs) = mapRight toStack $ parseVals "[clone] [clone]"
    ...
    it "evals `[clone] quote`" $ do
      ...
      let (Right vs) = mapRight toStack $ parseVals "[[clone]]"
      let ms = MultiStack (Map.singleton "$" vs)
      eval' e `shouldBe` ms
    ...
    it "evals `0 [clone] apply`" $ do
      ...
      let (Right vs) = mapRight toStack $ parseVals "0 0"
      let ms = MultiStack (Map.singleton "$" vs)
      eval' e `shouldBe` ms

    it "evals `[drop] (clone compose)`" $ do
      let (Right e) = parseExpr "[drop] (clone compose)"
      let (Right vs) = mapRight toStack $ parseVals "[drop drop]"
      let ms = MultiStack (Map.singleton "$" vs)
      eval' e `shouldBe` ms

    it "evals `True True and`" $ do
      let (Right e) = parseExpr "True True and"
      let (Right vs) = mapRight toStack $ parseVals "True"
      let ms = MultiStack (Map.singleton "$" vs)
      eval' e `shouldBe` ms
    ...
    it "evals `0 incr`" $ do
      ...
      let (Right vs) = mapRight toStack $ parseVals "1"
      let ms = MultiStack (Map.singleton "$" vs)
      eval' e `shouldBe` ms

    it "evals `1 decr`" $ do
      let (Right e) = parseExpr "1 decr"
      let (Right vs) = mapRight toStack $ parseVals "0"
      let ms = MultiStack (Map.singleton "$" vs)
      eval' e `shouldBe` ms

    it "evals `2 2 add`" $ do
      let (Right e) = parseExpr "2 2 add"
      let (Right vs) = mapRight toStack $ parseVals "4"
      let ms = MultiStack (Map.singleton "$" vs)
      eval' e `shouldBe` ms

    it "evals `4 2 sub`" $ do
      let (Right e) = parseExpr "4 2 sub"
      let (Right vs) = mapRight toStack $ parseVals "2"
      let ms = MultiStack (Map.singleton "$" vs)
      eval' e `shouldBe` ms
    ...
    it "evals `1 2 bit_or`" $ do
      ...
      let (Right vs) = mapRight toStack $ parseVals "3"
      let ms = MultiStack (Map.singleton "$" vs)
      eval' e `shouldBe` ms
    ...
    it "evals `5 5 lt`" $ do
      ...
      let (Right vs) = mapRight toStack $ parseVals "False"
      let ms = MultiStack (Map.singleton "$" vs)
      eval' e `shouldBe` ms
    ...
    it "evals `0 1 $a<- $b<- $a-> $b->`" $ do
      ...
      let (Right vs) = mapRight toStack $ parseVals "1 0"
      let ms = MultiStack (Map.singleton "$" vs)
      eval' e `shouldBe` ms
    ...
    it "evals fib" $ do
      ...
      let (Right vs) = mapRight toStack $ parseVals "8"
      let ms = MultiStack (Map.singleton "$" vs)
      eval (toEvalEnv env') ["$"] e (MultiStack Map.empty) `shouldBe` ms
    ...
    it "evals `B0 {match {case B0 => B1} {case B1 => B0}}`" $ do
      ...
      let (Right vs) = mapRight toStack $ parseVals "B1"
      let ms' = MultiStack (Map.singleton "$" vs)
      eval (toEvalEnv env) ["$"] e (MultiStack Map.empty) `shouldBe` ms'
    ...
    it "evals `0 True Pair {match {case Pair => }}`" $ do
      ...
      let (Right vs) = mapRight toStack $ parseVals "0 True"
      let ms' = MultiStack (Map.singleton "$" vs)
      eval (toEvalEnv env) ["$"] e (MultiStack Map.empty) `shouldBe` ms'
  ...
  describe "evalWithFuel" $ do
    ...
    it "evals `fib`" $ do
      let ([], env) = defineFns emptyEnv [swap, fib]
          ...
          n `inSteps` steps =
            let (Right vs) = mapRight toStack $ parseVals (show n)
